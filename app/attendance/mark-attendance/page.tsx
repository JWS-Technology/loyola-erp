...
export default function AttendancePage() {
  ...
  // 2. Log the data when the page loads
  useEffect(() => {
  ...
  useEffect(() => {
    if (!classId || !date || !hour) {
      // You might want to redirect back if data is missing
      // router.replace("/attendance/details");
      return;
    }

    // Pass the new filters to your API if needed
    fetch(`/api/students?class=${classId}&course=${course}&year=${year}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  }, [classId, date, hour, course, year, router]);
